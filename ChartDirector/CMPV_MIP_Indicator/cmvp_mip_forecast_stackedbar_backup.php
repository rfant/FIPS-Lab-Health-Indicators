...
{	for($i=0;$i<sizeof($Clean_Lab_Name_Array)-1;$i++)
	...
	// the CMVP ever starts to publish the Module_Type and SL on the MIP page, I'll be already to take advantage of it.
	...
	from(	SELECT 	\"Clean_Lab_Name\", 	AVG (\"RP_Value\") as rp_avg,avg(\"IR_Value\") as ir_avg,avg(\"CO_Value\")as co_avg FROM \"Model_Forecast_Confidence\"
	INNER JOIN \"CMVP_MIP_Table\" USING(\"Clean_Lab_Name\") GROUP BY \"Clean_Lab_Name\" ) as t2  	where \"CMVP_MIP_Table\".\"Clean_Lab_Name\" = t2.\"Clean_Lab_Name\"; 
 	...
 	UPDATE  \"CMVP_MIP_Table\" SET 	\"Y_RP_Avg\" = (case when \"Y_RP_Avg\" is null then 270 else \"Y_RP_Avg\" end) , 
								\"Y_IR_Avg\"=  (case when \"Y_IR_Avg\" is null then 72 else \"Y_IR_Avg\" end), 
								\"Y_CO_Avg\"=  (case when \"Y_CO_Avg\" is null then 99 else \"Y_CO_Avg\" end) ;";
