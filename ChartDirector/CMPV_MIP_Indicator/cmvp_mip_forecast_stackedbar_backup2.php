...
//already. The calculations are run once a day, and take several minutes to calculate. So, it's far better to do it once in the morning, and stored the
//information so I can retrieve it here later.

//echo "<br>size CLA=".sizeof($Clean_Lab_Name_Array)." size MT_Sec=".sizeof($MT_Security_Array)."<br>";


// lab and confidence array from "CMVP_Forecast_Confidence" SQL table.
for($i=0;$i<sizeof($Clean_Lab_Name_Array)-1;$i++)
	for($j=0;$j<sizeof($MT_Security_Array)-1;$j++)
		{
		
			$sql_Str_Confidence2=" select * from \"Model_Forecast_Confidence\" where \"Clean_Lab_Name\" like '%".$Clean_Lab_Name_Array[$i]."%' 
			AND \"MT_Security\" ='".$MT_Security_Array[$j]."' order by \"Row_ID\" desc limit 1 ;
			";

			//echo "hotel sql=<br>".$sql_Str_Confidence2."<br>";


			$result = pg_query($conn,$sql_Str_Confidence2);
			$arr = pg_fetch_all($result);
			if($arr==null)
				{  // echo "ERROR 400R:  SQL Query returned nothing. Sql=<br>".$sql_Str_Confidence2."</br>";
				$num_mod=0;
				}
			else
			{	$num_mod=sizeof($arr);
		...
		} // big for loop for lab and confidence array
...
{ 
  $where_vendor= " and  \"Vendor_Name\" like '%' ";
  $module_count=0;  //we can set the minimum number of modules a lab has done to be included in this plot
}
...
//adding 0.1 in the below SQL query means that it is running late. This allows me to add a "message" to the plot "running late"

$sql_Str_Forecast=" 
(
select \"Module_Name\",\"Clean_Lab_Name\",\"Vendor_Name\", \"Review_Pending_Start_Date\" as StartDate, rp_time_remaining, ".$forecast_Y_ir." as ir_time_remaining, ".$forecast_Y_co." as co_time_remaining from ( 
	select \"Module_Name\",\"Clean_Lab_Name\",\"Vendor_Name\", \"Review_Pending_Start_Date\", 
	case when (select (current_date)::date - (\"Review_Pending_Start_Date\" + INTERVAL '".$forecast_Y_rp." days')::date ) <0 then abs((select (current_date)::date - (\"Review_Pending_Start_Date\" + INTERVAL '".$forecast_Y_rp." days')::date ))  
	else
	(select (current_date)::date - (\"Review_Pending_Start_Date\" + INTERVAL '".$forecast_Y_rp." days')::date ) +.1 end  as rp_time_remaining 
...
UNION
(
select \"Module_Name\",\"Clean_Lab_Name\", \"Vendor_Name\",\"In_Review_Start_Date\" as StartDate , 0 as rp_time_remaining, ir_time_remaining, ".$forecast_Y_co." as co_time_remaining 
 from ( 
	select \"Module_Name\",\"Clean_Lab_Name\",\"Vendor_Name\", \"Review_Pending_Start_Date\",\"In_Review_Start_Date\", 
	 case when (select (current_date)::date - (\"In_Review_Start_Date\" + INTERVAL '".$forecast_Y_ir." days')::date )<0 then abs((select (current_date)::date - (\"In_Review_Start_Date\" + INTERVAL '".$forecast_Y_ir." days')::date ))  else
	 abs((select (current_date)::date - (\"In_Review_Start_Date\" + INTERVAL '".$forecast_Y_ir." days')::date )) + .1 end as ir_time_remaining 
...
( select \"Module_Name\",\"Clean_Lab_Name\",\"Vendor_Name\", \"Coordination_Start_Date\" as StartDate, 0 as rp_time_remaining, 0 as ir_time_remaining, co_time_remaining from ( 
	...
	case when ( select (current_date)::date - (\"Coordination_Start_Date\" + INTERVAL '".$forecast_Y_co." days')::date )<0 then abs(( select (current_date)::date - (\"Coordination_Start_Date\" + INTERVAL '".$forecast_Y_co." days')::date ))  else 
	abs(( select (current_date)::date - (\"Coordination_Start_Date\" + INTERVAL '".$forecast_Y_co." days')::date )) + .1 end as co_time_remaining 
...
//echo "delta sql =<br>".$sql_Str_Forecast."<BR>"; 
   	...
   	<tr> 	<td align="center">  <button type='submit' >    Refresh  </button> 
			...
			<td>
				...
				<script>
   				...
   				?>
   				 onclick="window.location.href='http:'+ Aurl+ '/ChartDirector/CMVP_MIP_Indicator/cmvp_mip_forecast_stackedbar.php?in_TopButtons=1&startDate='+ AstartDate+ '&endDate='+ AendDate;"> Last 24 Months  
   			...
   			<td> 
				<script>
   				...
   				if($in_TopButtons==2)
   					...
   					echo "<button  style=\"background-color: silver;\" type=\"button\" ";
   				?>
   				 onclick="window.location.href='http:'+ Burl+ '/ChartDirector/CMVP_MIP_Indicator/cmvp_mip_forecast_stackedbar.php?in_TopButtons=2&startDate='+BstartDate+ '&endDate='+BendDate;"> Last Year  
   				</button>  
   			</td> 
			<td>
				<script>
				...
				<?php
   				if($in_TopButtons==3)
   					...
   					echo "<button  style=\"background-color: silver;\" type=\"button\" ";
   				?>
   				 onclick="window.location.href='http:'+ C_url+ '/ChartDirector/CMVP_MIP_Indicator/cmvp_mip_forecast_stackedbar.php?in_TopButtons=3&startDate='+ CstartDate+ '&endDate='+ CendDate;"> This Year  
   				</button> 
   			</td>
   			<td>
				<script>
				...
				<?php
   				if($in_TopButtons==4)
   					...
   					echo "<button  style=\"background-color: silver;\" type=\"button\" ";
   				?>
   				  onclick="window.location.href='http:'+ Durl+ '/ChartDirector/CMVP_MIP_Indicator/cmvp_mip_forecast_stackedbar.php?in_TopButtons=4&startDate='+ DstartDate+ '&endDate=' + DendDate ;"> All Time  
   				</button> 
   			</td>
</tr>
